...
void Store::out(string name) {
    for (auto & m_Book : m_Books) {
        if (m_Book.get_name() == name) m_Book.SetID(0);
    }
    this->total--;
}
...
Book Store::findbyName(string name) {
    Book *t = nullptr;
    for (auto & m_Book : m_Books) {
        if (m_Book.get_name() == name) return m_Book;
    }
    return *t;
}
...
Book Store::findbyID(int id) {
    Book *t = nullptr;
    for (auto & m_Book : m_Books) {
        if (m_Book.GetID() == id) return m_Book;
    }
    return *t;
}
...
void Store::printList() {
    int n = 0;
    for (auto & m_Book : m_Books) {
        if (m_Book.GetID() == 0) continue;
        n++;
    }
    cout << "There are totally " << n << " Books:" << endl;
    for (auto & m_Book : m_Books) {
//        cout << m_pBook[i].GetID() << endl;
        if (m_Book.GetID() == 0) continue;
        cout << "ID=" << m_Book.GetID() << ";  ";
        cout << "Name:" << m_Book.get_name() << ";  ";
        cout << "Author:" << m_Book.get_author() << ";  ";
        cout << "Date:" << m_Book.getdate() << ";" << endl;
    }
 }
